...
function getTodayStr() {
  ...
  return { solar: `${y}년 ${m}월 ${d}일 (${week}요일)`, lunar: `음력 ${lm}월 ${ld}일`, jeolgi, y, m, d };
}
...
function getCategoryHint(userMessage) {
  if (/연애|사랑|좋아하|고백|사귀|설레|짝사랑|이별/.test(userMessage))
    return "연애 이야기를 할 때는 설렘과 불안이 공존하는 감각을 먼저 포착해요. 판단보다 공감이 먼저예요.";
  if (/이직|직장|커리어|일|승진|창업|취업|사직|퇴사/.test(userMessage))
    return "일과 커리어 이야기는 현실적인 무게감을 인정하면서도 가능성의 언어로 마무리해요. 막막함을 방향으로 바꿔주는 톤이에요.";
  if (/돈|재물|투자|부업|월급|빚|재정|집/.test(userMessage))
    return "돈 이야기는 불안을 건드리기 쉬우니 담담하게, 하지만 희망을 심는 방식으로 풀어요.";
  if (/건강|피곤|아프|스트레스|수면|몸|운동/.test(userMessage))
    return "건강 이야기는 몸의 신호를 감정의 언어로 번역해줘요. 몸과 마음이 연결되어 있다는 걸 자연스럽게 녹여요.";
  if (/인간관계|친구|가족|갈등|외로|사람|동료/.test(userMessage))
    return "관계 이야기는 상대방의 입장도 살짝 열어줘요. 일방적인 위로보다 관계의 입체감을 보여주는 톤이에요.";
  return "삶의 방향과 미래에 대한 이야기는 불확실함을 적으로 보지 않고 여정의 일부로 포용하는 톤으로 써요.";
}
...
━━━ 분량과 호흡 ━━━
공백 포함 약 900자 내외의 풍성한 분량으로 써요.
문단 하나는 최소 3-4문장으로, 호흡을 길게 가져가요.
