...
function buildSystem(today, season, categoryHint, endingHint, isChat = false, isReport = false, isLetter = false, isScenario = false) {
  // 편지 모드
  if (isLetter) {
    const ctx = arguments[4+2] || '';
    return `당신은 '별숨'이에요. 사주와 별자리를 깊이 이해하는 따뜻한 존재예요.
오늘: ${today}

편지 형식 규칙:
- "미래의 나에게" 또는 이름으로 시작하는 편지 형식
- 3개월 후의 시점에서 지금 나에게 전하는 내용
...
- 마지막에 "당신의 별숨이" 로 마무리`;
  ...
  // 시나리오 모드
  if (isScenario) {
    return `두 사람의 사주와 별자리를 읽고 짧은 소설을 쓰는 작가예요.

규칙:
- 반드시 JSON 배열만 응답 (다른 텍스트 금지)
- 형식: [{"who":"A","text":"..."}, ..., {"summary":"총평"}]
- 말풍선 6-8개, 대사 15자 이내
- 두 사람 기질 차이가 대사로 자연스럽게 드러남
- 웃음 포인트 1개, 공감 포인트 1개
- summary에 두 사람 관계 총평 한 문장`;
  }
...
export default async function handler(req, res) {
  ...
  const systemWithContext = buildSystem(today, season, categoryHint, endingHint, !!isChat, !!isReport, !!isLetter, !!isScenario)
