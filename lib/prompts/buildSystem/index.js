 ...
 */
export async function buildSystem(today, season, categoryHint, endingHint, timeHorizon, userMessage, isChat, isReport, isLetter, isScenario, isStory, isDecision, categoryExample = '', isNatal = false, isZodiac = false, isComprehensive = false, isAstrology = false, responseStyle = 'M', isSlot = false, isWeekly = false, isDaily = false, isDaeun = false, isAnalytics = false, precision_level = 'low', gender = null, lifeStage = 'free', isProphecy = false, isYearly = false, isFollowUpQ = false, isTarot = false, isDream = false, isName = false, isTaegil = false, isSajuChapter = false, isCalendarMonth = false) {
  let base;
  if (isFollowUpQ) {
    // 명시적 플래그 — userMessage 추론(isDecision 등)보다 항상 우선해야 한다.
    // 원래 질문에 "~될까/할까"가 있으면 decisionPrompt로 새던 라우팅 버그 방지.
    const { mainPrompt } = await import('./main.js');
    base = mainPrompt(
      today, season, categoryHint, endingHint, timeHorizon,
      false, isChat, categoryExample, responseStyle, true
    );
  } else if (isReport) {
    const { buildMonthlyPrompt } = await import('./monthly.js');
    base = buildMonthlyPrompt(today, responseStyle);
  } else if (isYearly) {
    ...
    base = buildDailyHoroscopeGamified(today, season, responseStyle, gender, lifeStage);
  ...
  } else if (isSajuChapter) {
    const { sajuStoryChapterPrompt } = await import('./story.js');
    base = sajuStoryChapterPrompt(today, responseStyle);
  } else if (isStory) {
    ...
    // 달력 패널은 원문을 그대로 렌더 — main의 [요약]/[점수] 태그가 노출되지 않게 전용 프롬프트 사용
    const { calendarMonthPrompt } = await import('./calendar.js');
    base = calendarMonthPrompt(today, responseStyle);
  } else if (isDecision && !isChat) {
    const { decisionPrompt } = await import('./decision.js');
    base = decisionPrompt(today, season, responseStyle);
  } else if (isTarot || isDream || isName || isTaegil) {
    const { buildSpecialFeaturePrompt } = await import('./special_feature.js');
    const type = isTarot ? 'tarot' : isDream ? 'dream' : isName ? 'name' : isTaegil ? 'taegil' : 'special';
    base = buildSpecialFeaturePrompt(today, type, responseStyle);
  } else {
    const { mainPrompt } = await import('./main.js');
    base = mainPrompt(
      today, season, categoryHint, endingHint, timeHorizon,
      false, isChat, categoryExample, responseStyle, isFollowUpQ
    );
  }
