import { getKstDateParts, getSolarTermInfo } from "../jeolgi.js";
import { formatLunarDate, solarToLunar } from "./lunar.js";
...
export function getTodayStr(clientHour) {
  ...
  const m = kst.month;
  const d = kst.day;
  const requestedHour = Number(clientHour);
  const h = Number.isFinite(requestedHour) && requestedHour >= 0 && requestedHour <= 23
    ? requestedHour
    : kst.hour;
  const week = ["일", "월", "화", "수", "목", "금", "토"][kst.weekIndex];
  const jeolgi = getSolarTermInfo(y, m, d, h, kst.minute).name;
  ...
  return {
    ...
    lunar: lunarStr,
    lunarSupported: lunar.supported,
    jeolgi,
    y,
    m,
    d,
...
⑤ 문장 끝: "~해요" "~이에요" — 단호하고 간결하게. "~합니다" "~습니다" "~됩니다" 체 금지.
[F형 전용 구조 오버라이드 없음 — 핵심 제언 첫 문장은 반드시 원인·결론·수치로 시작]
⑥ 전체 글은 "~해요/~이에요" 체로만 일관. "거입니다" 같은 잘못된 어미 금지.`;
  }
  if (responseStyle === 'F') {
    return `
...
④ 행동 제안 딱 하나. "~해도 괜찮아요" 또는 "~하는 것만으로 충분해요" 형식 (전역 어미 금지 규칙과 충돌하는 "~해봐도" "~해보는 건" 금지).
⑤ 마무리: "잘 될 거예요" 대신 "지금 [X]하는 게 맞는 방향이에요" 처럼 근거 있는 응원.
⑥ '포근한' '따스한' '별빛이' '흙처럼' '불꽃처럼' 등 시적 표현 절대 금지.
⑦ 전체 글은 "~해요/~이에요" 체로만 일관. "~합니다" "~습니다" "거입니다" 금지.`;
  }
  // 'M' - 균형형
  return `
...
export function isDecisionQuestion(userMessage) {
  ...
  return /갈까|할까|해도\s*(될까|괜찮을까|좋을까)|말할까|연락할까|시작할까|해야\s*(할까|하나)|가야\s*(할까|하나)|사도\s*(될까|괜찮을까)|만날까|헤어질까|바꿀까|그만할까|참을까|말해야\s*할까|해도\s*될까|가도\s*될까|(?:할지|갈지|다닐지|살지|만날지|헤어질지|시작할지|그만둘지).+(?:할지|갈지|다닐지|살지|만날지|헤어질지|시작할지|그만둘지)\s*(모르겠|고민|헷갈)/.test(userMessage);
}
