...
export function getStyleInstruction(responseStyle) {
  if (responseStyle === 'T') {
    return `
━━━ 응답 스타일: 분석형(T) ━━━
① 두괄식: 핵심 결론·확률·날짜를 첫 문장에. 공감 인사 전면 금지.
② "지금 이 시기는 ~한 특성이 있어요" 형태로 상황을 정의. 감정 묘사 없이 조건과 이유만.
③ 사주·별자리는 결론 뒤 근거로 — "이 기질이 ~한 결과를 만들어요" 형식.
④ 행동 제안: 날짜·횟수·금액 등 수치 포함. 날짜는 향후 날짜 목록에서 선택(이미 지난 요일 언급 금지), "하루 1번", "5만원 이하로".
⑤ "다 잘 될 거예요" 등 막연한 위로·시적 비유 전면 금지.
⑥ 문장 끝: "~해요" "~이에요" "~됩니다" — 단호하고 간결하게.`;
  }
  if (responseStyle === 'F') {
    return `
━━━ 응답 스타일: 공감형(F) ━━━
① 두괄식 유지: [요약]·첫 문단에 핵심 방향 먼저. 공감은 2번째 문장부터.
② 공감 언어: "지금 많이 힘드셨겠어요", "그럴 수 있어요", "충분히 이해돼요" — 구체적 상황에 연결.
③ 시적 비유·감성 묘사 금지. '포근한' '따스한' '별빛이' '흙처럼' '불꽃처럼' 절대 금지.
④ 사주·별자리는 "지금 이 상황을 이해하는 데" 도움이 된다는 관점으로. 추상 묘사 금지.
⑤ 행동 제안 딱 하나. "~해봐도 좋을 것 같아요" 또는 "~해보는 건 어때요" 형식.
⑥ 마지막은 위로·응원으로 — "잘 될 거예요" 대신 "지금 이 선택이 맞는 방향이에요" 처럼 근거 있는 응원.`;
  }
  // 'M' - 균형형
  return `
━━━ 응답 스타일: 균형형(M) ━━━
