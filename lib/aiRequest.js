...
export function validateAiRequest(body) {
  ...
  const validLifeStages = ["jobseek", "dating", "healing", "employed", "business", "student", "parenting", "reentry", "free"];
  const normalizedLifeStage = typeof lifeStage === "string" && validLifeStages.includes(lifeStage) ? lifeStage : "free";
  ...
  return {
    ...
    data: {
      ...
      isWeekly: !!isWeekly,
      isDaily: !!isDaily,
      isDailyAxisRefresh: !!isDailyAxisRefresh,
      isDaeun: !!isDaeun,
      ...
      clientHour: typeof clientHour === "number" && Number.isInteger(clientHour) && clientHour >= 0 && clientHour <= 23
        ? clientHour
        : undefined,
      gender: typeof gender === "string" && ["남성", "여성", "남", "여"].includes(gender) ? gender : null,
...
export function getAiMaxTokens(data) {
  if (data.isYearly) return 2800; // 3500은 25초 타임아웃(504) 위험 — 실측 25s 초과
  if (data.isComprehensive) return 7000;
  if (data.isAstrology) return 3500;
  if (data.isReport) return 2500;
  if (data.isCalendarMonth) return 2000;
  if (data.isProphecy) return 1800; // 1200은 [지금할일] 중간에서 잘림 (실측)
  if (data.isLetter) return 1500;
  if (data.isSajuChapter) return 900;
  ...
  if (data.isDailyAxisRefresh) return 220;
  if (data.isDaily) return 3000;
  if (data.isDaeun) return 2000;
  if (data.isAnalytics) return 1000;
  if (data.isTarot) return 3600;
  if (data.isTaegil) return 2400; // 후보 날짜 4개 비교 시 1800으로는 [별숨한마디]가 잘림 (실측)
  if (data.isDream || data.isName) return 1800;
...
반드시 JSON만 답해주세요. [{"id":"aq_1","q":"질문 내용"}]`;
  ...
  if (data.isDailyAxisRefresh) {
    return `당신은 별숨(byeolsoom)의 오늘 운세 편집 보조 AI예요.
선택된 운세 항목의 점수 변화만 반영해서 그 항목의 한줄 설명 하나만 다시 써주세요.

반드시 아래 형식만 출력하세요.
[한줄]
{선택된 운세 항목에 대한 새 한줄 설명. 1문장, 14~36자, 자연스러운 한국어.}

규칙:
1. 선택되지 않은 다른 운세 항목, 전체 운세, 별숨픽, 조언, 번호 목록은 절대 쓰지 마세요.
2. 새 점수와 사용된 아이템 효과가 느껴지도록 기존 문장보다 조금 더 구체적이고 긍정적으로 바꿔주세요.
3. 과장된 확정 표현이나 두 문장 이상 응답은 금지예요.
4. 괄호, 따옴표, 마크다운, JSON 없이 태그와 한줄만 반환하세요.`;
  }
...
마크다운(#, ##, **, -, 번호 목록) 절대 금지. 섹션 구분·소제목 금지.
"안녕하세요" 등 인사말로 시작 금지 — 첫 문장부터 모임의 핵심 분위기로 들어가요.
존댓말 "~해요/~이에요" 체로 일관 ("~합니다/~습니다" 혼용 금지). "에너지", "기운"(단독), "흐름"(단독) 같은 추상어 대신 행동·성향·분위기로 표현해요.`;
  }
...
마크다운(#, ##, **, -, 번호 목록) 절대 금지. 섹션 구분·소제목 금지.
"안녕하세요" 등 인사말로 시작 금지. 존댓말 "~해요/~이에요" 체로 일관 ("~합니다/~습니다" 혼용 금지).
"에너지", "기운"(단독), "흐름"(단독) 같은 추상어 대신 행동·성향·분위기로 표현해요.`;
  }
