...
function normalizeBirthTime(birthHour = 12, birthMin = 0) {
  const hourValue = Number(birthHour);
  const minValue = Number(birthMin);
  if (!Number.isFinite(hourValue)) return { hour: 12, minute: 0 };

  const wholeHour = Math.trunc(hourValue);
  const hasExplicitMinute = Number.isFinite(minValue) && Math.abs(minValue) > 0;
  const minuteFromHour = hasExplicitMinute ? 0 : Math.round((hourValue - wholeHour) * 60);
  const totalMinutes = wholeHour * 60 + minuteFromHour + (Number.isFinite(minValue) ? minValue : 0);
...
function normalizeGender(gender = 'M') {
  ...
  if (value === 'm' || value === 'male' || value === '남성' || value === '남') return 'M';
  if (value === 'f' || value === 'female' || value === '여성' || value === '여') return 'F';
  return null;
}
 ...
 */
function calcStartAge(birthYear, birthMonth, birthDay, birthHour, birthMin, isForward) {
...
export function getDaeun(birthYear, birthMonth, birthDay, birthHour = 12, birthMin = 0, gender = 'M') {
  ...
  // 월지 인덱스 (절기 기준)
  const { hour, minute } = normalizeBirthTime(birthHour, birthMin);
  const wj = getMonthJijiIndex(birthYear, birthMonth, birthDay, hour, minute);
  // 월간 인덱스 (오호둔월법)
  ...
  const isForward  = normalizedGender === 'M'
    ? isYangYear
    : normalizedGender === 'F'
      ? !isYangYear
      : false;

  const startAge = calcStartAge(birthYear, birthMonth, birthDay, hour, minute, isForward);
