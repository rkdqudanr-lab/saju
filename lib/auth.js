...
// 동일 네트워크 대역 여부 판정.
// 휴대폰은 LTE↔WiFi 전환·셀룰러 IP 변동으로 접속 IP가 수시로 바뀌므로
// 정확한 IP 일치 대신 대역(IPv4 /16, IPv6 앞 2블록)만 비교해 세션 유지력을 높인다.
function sameIpNetwork(a, b) {
  if (!a || !b) return true;
  if (a === b) return true;
  ...
  if (x.includes(":") && y.includes(":")) {
    const px = x.split(":").slice(0, 2).join(":");
    const py = y.split(":").slice(0, 2).join(":");
    return px === py;
  }
  if (!x.includes(":") && !y.includes(":")) {
    const px = x.split(".").slice(0, 2).join(".");
...
async function refreshAuthSession(req, res) {
  ...
  try {
    ...
    if (session.ip_address && requestIp && !sameIpNetwork(session.ip_address, requestIp)) {
      clearAuthCookies(res);
      return { ok: false };
    }
...
export function verifySessionToken(req) {
  ...
  if (!jwtSecret) {
    // 프로덕션에서 JWT_SECRET 누락은 설정 오류 — 무인증 통과를 막는다.
...
export async function authenticateRequest(req, res, options = {}) {
  ...
  if (sessionResult.ok) {
    // 슬라이딩 창: 유효한 액세스 세션을 확인할 때마다 쿠키를 30일로 다시 채운다.
    // 같은 기기에서 30일 안에 한 번이라도 들어오면 만료가 계속 미뤄져 로그아웃되지 않는다.
    if (sessionResult.kakaoId) {
      const slidToken = issueAccessToken(sessionResult.kakaoId);
      if (slidToken) setAccessCookie(res, slidToken);
    }
