...
### 1. 기능별 코드 분리 원칙
- 모든 코드는 **기능 단위로 분리**해서 작성한다
- 하나의 파일/함수에 여러 기능을 몰아넣지 않는다
- 새 기능 추가 시 → 기존 파일에 무작정 추가하지 말고, 역할에 맞는 파일/컴포넌트에 분리 배치
- 예시:
  - 데이터 fetch 로직 → `hooks/` 또는 `lib/` 분리
  - UI 컴포넌트 → `components/` 또는 `pages/` 분리
  - 계산/유틸 로직 → `utils/` 분리
  - API 엔드포인트 → `api/` 분리

### 2. 먼저 질문하고, 확인 후 실행
- 지시가 **무슨 뜻인지 모르거나 범위가 애매할 때**는 실행 전 반드시 먼저 물어본다
- 지레 짐작하여 엉뚱한 코드를 작성하지 않는다
- 특히 기존 코드에 큰 영향을 줄 수 있는 변경은 계획을 먼저 공유하고 승인받는다

### 3. 분석 범위 제한
- **지시한 파일/기능의 코드만** 읽고 수정한다
- 전체 코드베이스를 무차별적으로 분석하지 않는다
- 수정 대상과 직접 관련된 파일만 참고한다
- 추가 탐색이 필요하면 이유를 먼저 설명하고 진행한다
