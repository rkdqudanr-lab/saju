  ...
  return (
    <div className="page step-fade">
      <div className="land-hero">
        ...
        <p className="land-copy" style={{ fontSize: 'var(--md)', lineHeight: 1.7, fontWeight: 500, letterSpacing: '-0.01em', color: 'var(--t2)' }}>
          당신의 별숨은 당신을 바라보고 있어요.
        </p>
        <p className="land-beta-notice" style={{ fontSize: 'var(--xxs)', opacity: 0.7, marginTop: 'var(--sp1)', letterSpacing: '0.02em' }}>
        ...
        {isApproximate && (
          <div
            ...
            style={{ padding: 'var(--sp2) var(--sp3)', cursor: 'pointer', textAlign: 'center', margin: '0 0 var(--sp3)' }}
          >
            지금은 추정 사주로 보고 있어요.<br />
            <strong style={{ fontSize: 'var(--xxs)', opacity: 0.8 }}>생일 일자를 추가하면 더 정확해져요 →</strong>
          </div>
        )}

        <div className="land-login-section" style={{ animationDelay: '0.2s' }}>
          {user ? (
            <div className="land-login-card logged">
              <GamificationHeaderV2
                ...
                style={{ marginBottom: 12, borderRadius: 'var(--r1)' }}
              />

              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                {user.profileImage
                  ? <img className="llc-avatar" src={user.profileImage} alt="프로필" style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover', border: '0.5px solid var(--line)' }} />
                  : <div className="llc-avatar-placeholder" style={{ width: 44, height: 44, borderRadius: '50%', background: 'var(--bg3)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '0.5px solid var(--line)', fontSize: 'var(--sm)' }}>🌙</div>}
                <div style={{ flex: 1, textAlign: 'left' }}>
                  <div className="llc-name" style={{ fontSize: 'var(--sm)', fontWeight: 700, color: 'var(--t1)', marginBottom: 4 }}>{user.nickname} <span style={{ color: 'var(--gold)', fontSize: 'var(--xxs)' }}>✦</span></div>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => { setEditingMyProfile(true); setStep(1); }} className="res-btn" style={{ padding: '4px 10px', minWidth: 'auto', fontSize: '10px', fontWeight: 600, background: 'var(--bg2)', border: '0.5px solid var(--line)', borderRadius: 50 }}>수정</button>
                    <button onClick={kakaoLogout} className="res-btn" style={{ padding: '4px 10px', minWidth: 'auto', fontSize: '10px', fontWeight: 600, background: 'var(--bg2)', border: '0.5px solid var(--line)', borderRadius: 50 }}>로그아웃</button>
                  </div>
                </div>
              </div>
                ...
                {form.by ? (
                  <>
                    {(() => {
                        ...
                        return (
                          <>
                            <button
                              ...
                              className="q-item"
                              style={{ width: '100%', margin: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 'var(--xs)', fontWeight: 600, background: 'var(--bg-glass)', border: '0.5px solid var(--line)', padding: '12px 16px', borderRadius: 'var(--r1)', backdropFilter: 'blur(8px)' }}
                            >
                              <span style={{ color: 'var(--t1)' }}>✦ 오늘 하루 나의 별숨 · {today.month}월 {today.day}일</span>
                              <span style={{ fontSize: '10px', opacity: 0.6, color: 'var(--t4)' }}>{showDailyCard ? '▲ 접기' : '▼ 보기'}</span>
                            </button>
                            {showDailyCard && (
                                ...
                                {dailyLoading ? (
                                  ...
                                  <>
                                    ...
                                    {dailyCount < DAILY_MAX ? (
                                      <button className="up-btn" style={{ width: '100%' }} onClick={askDailyHoroscope}>
                                        다시 물어보기 ✦ ({dailyCount}/{DAILY_MAX})
                                      </button>
                                    ) : (
                                      <div style={{ textAlign: 'center', fontSize: 'var(--xs)', color: 'var(--t4)', marginTop: 4 }}>오늘 별숨을 모두 읽었어요 · 내일 다시 만나요 🌙</div>
                                    )}
                                  </>
                                ) : (
                                  <button className="cta-main" style={{ width: '100%', justifyContent: 'center' }} onClick={askDailyHoroscope}>오늘 기운 확인하기 ✦</button>
                                )}
                              </div>
                            )}
                              ...
                              <div style={{ background: 'var(--bg2)', borderRadius: 'var(--r1)', border: '1px solid var(--line)', overflow: 'hidden' }}>
                                <div style={{ padding: '12px 14px 10px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                    <span>📓</span>
                                    <span style={{ fontSize: 'var(--xxs)', color: 'var(--t1)', fontWeight: 700, letterSpacing: '-0.01em' }}>나의 하루를 별숨에게</span>
                                  </div>
                                  {diaryReviewResult && !diaryReviewLoading && (
                                    <button onClick={resetDiaryReview} className="hist-del-btn" style={{ padding: '3px 10px' }}>다시 쓰기</button>
                                  )}
                                </div>
                                <div style={{ padding: '12px 14px' }}>
                                  {diaryReviewLoading ? (
                                    ...
                                    <>
                                      ...
                                      <button onClick={() => setStep(17)} className="skip-btn" style={{ width: '100%' }}>별숨의 해석 전체 보기 →</button>
                                    </>
                                  )}
                                </div>
                              </div>
                      ...
                      return (
                        <>
                          <div style={{ fontSize: 'var(--xs)', color: 'var(--t4)', letterSpacing: '.06em', paddingTop: 6 }}>
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ background: 'var(--bg2)', borderRadius: 'var(--r1)', padding: '16px', border: '1px solid var(--line)', marginTop: 10 }}>
                          <div style={{ fontSize: 'var(--xxs)', color: 'var(--gold)', fontWeight: 700, marginBottom: 8, letterSpacing: '.06em', textTransform: 'uppercase' }}>
                            ✦ 오늘 별숨의 질문 {answeredCount > 0 && <span style={{ fontWeight: 400, color: 'var(--t4)', marginLeft: 6, textTransform: 'none' }}>{answeredCount}개 답했어요</span>}
                          </div>
                          ...
                          {allDone ? (
                            ...
                            <>
                              ...
                              <div style={{ fontSize: 'var(--sm)', color: 'var(--t1)', fontWeight: 500, lineHeight: 1.6, marginBottom: 4 }}>"{q.q}"</div>
                              <div style={{ fontSize: 'var(--xxs)', color: 'var(--t4)', lineHeight: 1.6, marginBottom: 12, opacity: 0.8 }}>{q.sub}</div>
