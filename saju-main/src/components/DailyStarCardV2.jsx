      ...
      <div className="dsc-card">
        ...
        {hasBadtime && (
          <div className="dsc-badtime">
            <div className="dsc-badtime-title">
              <span>🛡️</span> 별숨 궤도 보호
            </div>

            {badtime.symptom && (
              <div className="dsc-badtime-symptom">
                감지된 악운: {badtime.symptom}
              </div>
            )}
            ...
            {!blocked && (
              <button
                className="dsc-badtime-btn"
                onClick={handleBlockBadtime}
                disabled={isBlocking || !canBlockBadtime || currentBp < 20}
              >
                {isBlocking ? '보호막 발동 중...' : '액막이 발동 (-20 BP)'}
              </button>
            )}

            {/* 액막이 완료 메시지 */}
            {blocked && (
              <div className="dsc-badtime-done">
            ...
            {!canBlockBadtime || currentBp < 20 ? (
              <div className="dsc-badtime-hint">
                BP를 충전하거나 미션을 완료하면 보호를 발동할 수 있어요
              </div>
            ) : null}

            {/* 사주/별자리 근거 */}
            {(sajuReason || astrologyReason) && (
              <div className="dsc-badtime-logic">
