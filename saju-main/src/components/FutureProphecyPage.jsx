...
import { shouldDropCap } from "../utils/constants.js";
      ...
      <div className="page-top">
        <div className="inner" style={{ animation: 'fadeUp .5s ease' }}>
          <div style={{ textAlign: 'center', marginBottom: 'var(--sp3)' }}>
            ...
            <div style={{ fontSize: 'var(--xl)', fontWeight: 700, color: 'var(--gold)', letterSpacing: '-0.02em' }}>별숨의 예언</div>
            <div style={{ fontSize: 'var(--xxs)', color: 'var(--t4)', marginTop: 8, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{period.label} · {period.desc}</div>
          </div>

          <div className="letter-envelope">
            <div className="letter-env-top" style={{ background: 'linear-gradient(145deg, var(--goldf) 0%, rgba(155, 142, 196, 0.12) 100%)', boxShadow: 'inset 0 0 20px rgba(232, 176, 72, 0.1)' }}>
              <span style={{ filter: 'drop-shadow(0 0 8px rgba(232, 176, 72, 0.3))' }}>{period.emoji}</span>
            </div>
            <div className="letter-body">
              ...
              {loading ? (
                ...
                <div className="letter-content" style={{ padding: 0 }}>
                  <p className={shouldDropCap(shown) ? 'drop-cap' : ''}>{shown}{!done && <span className="typing-cursor" />}</p>
                </div>
              )}
            </div>
          </div>
    ...
    <div className="page" style={{ justifyContent: 'center' }}>
      <div className="inner" style={{ animation: 'fadeUp .6s ease' }}>
        ...
        <div className="pro-header">
          ...
          <div style={{ fontSize: 'var(--sm)', color: 'var(--t3)', lineHeight: 1.85, letterSpacing: '-0.01em' }}>
            별이 품은 시간의 이야기를<br />
            지금 당신에게 전해드릴게요
          </div>
        </div>
        ...
        {/* 핵심 질문 */}
        <div className="pro-q-card">
          <div className="pro-q-label">
            ✦ 오늘의 질문
          </div>
          <div className="pro-q-text">
            내가 알고 싶은<br />미래는?
          </div>
          <div className="pro-q-sub">
        ...
        <div style={{ marginBottom: 'var(--sp3)' }}>
          ...
          <div className="pro-period-list">
            {PERIOD_OPTIONS.map(p => (
              <button
                key={p.id}
                onClick={() => setPeriod(p.id)}
                className={`pro-period-btn ${selectedPeriod === p.id ? 'active' : ''}`}
              >
                <span className="pro-period-emoji">{p.emoji}</span>
                <div style={{ flex: 1 }}>
                  <div className="pro-period-title">
                    {p.label}
                  </div>
                  <div className="pro-period-desc">{p.desc}</div>
                </div>
                {selectedPeriod === p.id && (
