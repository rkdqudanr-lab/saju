  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <span style={{ fontSize: 'var(--lg)' }}>{icon}</span>
        <span style={{ fontSize: 'var(--sm)', fontWeight: 700, color: 'var(--t1)', letterSpacing: '-0.01em' }}>{title}</span>
      </div>
      <p style={{ margin: 0, fontSize: 'var(--sm)', color: 'var(--t2)', lineHeight: 1.85, whiteSpace: 'pre-wrap', wordBreak: 'keep-all', letterSpacing: '-0.005em' }}>
        {text}
      </p>
    </div>
  );
      ...
      <div className="inner">
        <div style={{ paddingBottom: 40 }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <div style={{ fontSize: 'var(--xl)', marginBottom: 8, color: 'var(--gold)' }}>✦</div>
            <div style={{ fontSize: 'var(--lg)', fontWeight: 800, color: 'var(--t1)', letterSpacing: '-0.02em', marginBottom: 8 }}>
              별숨의 종합 사주
            </div>
            <div style={{ fontSize: 'var(--sm)', color: 'var(--t3)', lineHeight: 1.7, fontWeight: 400 }}>
            ...
            {saju && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: 4, marginTop: 16, flexWrap: 'wrap' }}>
                {[
                  ...
                  <div key={label} style={{ background: 'var(--bg-glass)', border: '0.5px solid var(--line)', borderRadius: 8, padding: '4px 10px', fontSize: 'var(--xxs)', color: 'var(--t2)', fontWeight: 500 }}>
                    <span style={{ color: 'var(--gold)', fontWeight: 700, marginRight: 4 }}>{label}</span>{val}
                  </div>
                ))}
              </div>
            )}
          ...
          {hasContent && (
            <>
              <div style={{ marginBottom: 20, padding: '16px', background: 'var(--bg-glass-heavy)', borderRadius: 'var(--r2)', border: '0.5px solid var(--line)', backdropFilter: 'blur(10px)', boxShadow: 'var(--shadow)' }}>
                <div style={{ fontSize: '10px', color: 'var(--gold)', fontWeight: 700, marginBottom: 4, letterSpacing: '0.1em', textTransform: 'uppercase' }}>✦ Full Analysis Summary</div>
                <div style={{ fontSize: 'var(--xxs)', color: 'var(--t3)', fontWeight: 400 }}>타고난 본성부터 올해의 흐름까지</div>
              </div>
