  ...
  if (phase === 'done') {
    return (
      <div className="profile-overlay" role="dialog" aria-modal="true" aria-label="별숨에게 나를 알려주기"
        ...
        <div className="profile-sheet" ref={sheetRef} style={{ textAlign: 'center', padding: '48px 24px' }}>
          <div className="pro-orb">✦</div>
          <div style={{ fontSize: 'var(--md)', fontWeight: 800, color: 'var(--t1)', marginBottom: 12, letterSpacing: '-0.02em' }}>
            별숨과의 연결이 완성됐어요
          </div>
          <div style={{ fontSize: 'var(--sm)', color: 'var(--t3)', lineHeight: 1.8, marginBottom: 40 }}>
            {answeredCount}개의 깊은 이야기를 들었어요.<br />
            당신의 모든 운세에 이 진동이 반영될 거예요.
          </div>
  ...
  return (
    <div className="profile-overlay" role="dialog" aria-modal="true" aria-label="별숨에게 나를 알려주기"
      ...
      <div className="profile-sheet" ref={sheetRef}>
        ...
        {/* 헤더 */}
        <div style={{ padding: '0 4px 20px', marginBottom: 24, borderBottom: '0.5px solid var(--line)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div className="profile-title">
              {phase === 'ai' ? '맞춤 질문' : '별숨에게 나를 알려주기'}
            </div>
            <div style={{ fontSize: '11px', color: 'var(--t4)', fontFamily: 'var(--ff-inter)', fontWeight: 700 }}>
              {phase === 'ai'
          ...
          {/* 진행 바 */}
          <div style={{ height: 6, background: 'var(--bg-glass-heavy)', borderRadius: 3, overflow: 'hidden', border: '0.5px solid var(--line)' }}>
            <div style={{
          ...
          {phase === 'ai' && (
            <div style={{ fontSize: '10px', color: 'var(--gold)', marginTop: 8, fontWeight: 700, letterSpacing: '0.02em' }}>
              🌟 별숨이 만든 당신만의 맞춤 질문
            </div>
          )}
        </div>
        ...
        {(activeQ.type === 'text' || activeQ.type === 'mixed' || (phase === 'ai' && !activeQ.type)) && (
          <div style={{ marginTop: 0 }}>
            <textarea
              ...
              placeholder={activeQ.placeholder || '자유롭게 당신의 이야기를 들려주세요'}
              value={textInput}
              maxLength={300}
              onChange={e => setTextInput(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); answerText(); } }}
              style={{ minHeight: 100, marginBottom: 8, background: 'var(--bg-glass)', border: '0.5px solid var(--line)' }}
            />
            <div style={{ fontSize: '10px', color: 'var(--t4)', fontFamily: 'var(--ff-inter)', textAlign: 'right', marginBottom: 12 }}>
