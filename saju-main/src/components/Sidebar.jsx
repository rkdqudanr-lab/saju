  ...
  return (
    <>
      ...
      <nav className="sidebar" aria-label="주 네비게이션" role="navigation">
          ...
          {user ? (
            ...
            <div className="sidebar-user">
              ...
              <div>
                ...
                <div className="sidebar-usub" style={{ cursor: 'pointer', color: 'var(--gold)', fontWeight: 700 }} onClick={onKakaoLogin}>별숨과 함께하기 →</div>
              </div>
            </div>
          )}
        </div>
            ...
            <div className="sidebar-section">
              ...
              {openGroups.today && (
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  {[
                    { icon: '✦', label: '대시보드', s: 0 },
                    { icon: '◦', label: '오늘의 운세', s: 'fortune' },
                    { icon: '─', label: '별숨 일기', s: 17, badge: todayDiaryWritten === false },
                    { icon: '□', label: '별숨 달력', s: 10 },
                    { icon: '◇', label: '사주 명함', s: 21 },
                  ].map(m => (
                    <li key={m.s}>
                      <button
        ...
        <div className="sidebar-foot" style={{ padding: '24px', borderTop: '0.5px solid var(--line)', background: 'var(--bg-glass)' }}>
          {user && onInvite && (
            <button className="sidebar-foot-btn" style={{ background: 'var(--gold-grad)', border: 'none', color: '#000', marginBottom: 12, fontWeight: 800, padding: '14px', borderRadius: '12px', boxShadow: '0 4px 15px var(--gold-glow)' }} onClick={() => { onInvite(); onClose(); }}>
              🔗 친구 초대하기
            </button>
          )}
          {user && <button className="sidebar-foot-btn" style={{ border: '0.5px solid var(--line)', color: 'var(--t4)', fontSize: '11px', fontWeight: 600, letterSpacing: '0.02em' }} onClick={() => { onKakaoLogout(); onClose(); }}>로그아웃</button>}
        </div>
      </nav>
    </>
  );
