...
function scoreColor(score) {
  if (score >= 85) return '#6BD2A3'; // Vibrant Mint
  if (score >= 68) return '#94D994'; // Soft Green
  if (score >= 50) return '#E6C35A'; // Classic Gold
  if (score >= 38) return '#FF9F7D'; // Salmon
  return '#FF7D7D'; // Pale Rose
}
    ...
    <div className="page step-fade">
      <div className="inner">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, padding: '0 4px' }}>
          <button onClick={prevMonth} style={{ background: 'var(--bg-glass)', border: '0.5px solid var(--line)', borderRadius: 12, width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--t2)', fontSize: 'var(--lg)' }}>‹</button>
          <div style={{ fontSize: 'var(--md)', fontWeight: 800, color: 'var(--t1)', letterSpacing: '-0.01em', fontFamily: 'var(--ff-inter)' }}>{viewYear}. {String(viewMonth).padStart(2, '0')}</div>
          <button onClick={nextMonth} style={{ background: 'var(--bg-glass)', border: '0.5px solid var(--line)', borderRadius: 12, width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--t2)', fontSize: 'var(--lg)' }}>›</button>
        </div>
          ...
          {daysData.map(({ d, saju, score }) => {
            ...
            return (
              <button
                ...
                style={{
                  background: isSel ? 'var(--gold-grad)' : scoreBg(score),
                  border: isT ? `1.5px solid var(--gold)` : isSel ? 'none' : `0.5px solid ${sCol}33`,
                  borderRadius: 12,
                  padding: '8px 2px',
                  cursor: 'pointer',
                  color: isSel ? '#000' : weekday === 0 ? 'var(--rose)' : weekday === 6 ? '#4A8EC4' : 'var(--t1)',
                  display: 'flex',
                  ...
                  fontWeight: isT || isSel ? 800 : 500,
                  minHeight: 64,
                  transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)',
                  position: 'relative',
                  backdropFilter: 'blur(4px)',
                  boxShadow: isSel ? '0 4px 12px rgba(230,195,90,0.3)' : 'none',
                  transform: isSel ? 'scale(1.05)' : 'none',
                  zIndex: isSel ? 2 : 1,
                }}
              >
                <span style={{ fontSize: 'var(--xs)' }}>{d}</span>
                {saju && (
                  <span style={{ fontSize: '9px', color: isSel ? '#000' : 'var(--t4)', lineHeight: 1, fontWeight: 600, opacity: 0.8 }}>
                    {saju.il.gh}{saju.il.jh}
                  </span>
                )}
                <span style={{ fontSize: '10px', color: isSel ? '#000' : sCol, fontWeight: 800 }}>
                  {score}
                </span>
                {/* 기록 인디케이터 */}
                {(hasEvents || hasFortune || hasDiary) && (
                  <div style={{ display: 'flex', gap: 3, alignItems: 'center', marginTop: 1 }}>
                    {hasFortune && (
                      <span style={{ width: 4, height: 4, borderRadius: '50%', background: isSel ? '#000' : 'var(--gold)', display: 'block' }} />
                    )}
                    {hasDiary && (
                      <span style={{ width: 4, height: 4, borderRadius: '50%', background: isSel ? '#000' : '#B39DDB', display: 'block' }} />
                    )}
                    {hasEvents && (
                      <span style={{ width: 4, height: 4, borderRadius: '50%', background: isSel ? '#000' : '#80CBC4', display: 'block' }} />
                    )}
                  </div>
                )}
              </button>
