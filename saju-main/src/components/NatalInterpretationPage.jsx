      ...
      <div style={{ display: 'flex', gap: 10, paddingBottom: 4, width: 'max-content' }}>
        {PILLAR_META.map(({ key, label, role, desc }) => {
          ...
          return (
            ...
            >
              <div style={{ fontSize: 'var(--xxs)', color: 'var(--gold)', fontWeight: 700, letterSpacing: '.1em', marginBottom: 8, textTransform: 'uppercase' }}>{label}</div>
              <div style={{ fontSize: 'var(--xl)', fontWeight: 800, letterSpacing: '.04em', color: 'var(--t1)', lineHeight: 1.1, marginBottom: 4 }}>
                {p.gh}<br />{p.jh}
              </div>
              <div style={{ fontSize: 'var(--xxs)', color: 'var(--t4)', marginBottom: 12, fontWeight: 500 }}>{p.g}{p.j}</div>
              <div style={{ fontSize: '9px', color: 'var(--gold)', fontWeight: 700, letterSpacing: '.05em', marginBottom: 4, opacity: 0.8 }}>{role}</div>
              <div style={{ fontSize: '10px', color: 'var(--t3)', lineHeight: 1.6, wordBreak: 'keep-all' }}>{desc}</div>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="page step-fade">
      <div className="inner" style={{ paddingTop: 0 }}>
        ...
        <div
          style={{
            background: `linear-gradient(165deg, ${elTheme.border}15 0%, rgba(255,255,255,0.02) 100%)`,
            border: `0.5px solid ${elTheme.border}40`,
            borderRadius: 28,
        ...
        >
          <div style={{ fontSize: '10px', color: domColor, fontWeight: 700, letterSpacing: '.15em', marginBottom: 8, opacity: .8, textTransform: 'uppercase' }}>
            My Stellar Configuration
          </div>
          {saju && (
            <>
              <div style={{ fontSize: 'var(--xl)', fontWeight: 800, color: 'var(--t1)', marginBottom: 6, letterSpacing: '-0.02em', lineHeight: 1.3 }}>
                {saju.ilganPoetic}
              </div>
              <div style={{ fontSize: 'var(--sm)', color: 'var(--t3)', marginBottom: 20, lineHeight: 1.7, fontWeight: 400 }}>
          ...
          {['사주 원국', '별자리'].map((label, i) => (
            <button
              ...
              style={{
                ...
                boxShadow: tab === i ? '0 4px 12px rgba(0,0,0,.08)' : 'none',
              }}
            >
              {label}
            </button>
