...
import { PERSONAS } from "../utils/personaData.js";
      ...
      <div className="inner" style={{ paddingTop: 16, paddingBottom: 60 }}>
          ...
          {['개인정보', '요금제', '스타일', '메뉴'].map((label, i) => (
            <button
              ...
              style={{
                ...
                background: tab === i ? 'var(--gold-grad)' : 'transparent',
                color: tab === i ? '#000' : 'var(--t4)',
                boxShadow: tab === i ? '0 4px 12px var(--gold-glow)' : 'none',
              }}
            >
              {label}
            </button>
        ...
        {tab === 0 && (
          <div className="card" style={{ gap: 'var(--sp2)' }}>
            <div className="card-title">개인정보 관리</div>
            <div className="card-sub">별숨이 당신을 더 정확히 이해할 수 있도록 정보를 관리하세요</div>
                  ...
                  {STYLE_OPTIONS.map((opt, i) => (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 16,
                        position: 'relative',
                        boxShadow: responseStyle === opt.value ? '0 4px 15px var(--gold-glow)' : 'none',
                      }}
                    >
                      <span style={{ fontSize: '1.25rem', filter: responseStyle === opt.value ? 'none' : 'grayscale(1) opacity(0.5)' }}>{opt.emoji}</span>
                      <span style={{
                        fontSize: '11px',
                        fontWeight: 800,
                        color: responseStyle === opt.value ? '#000' : 'var(--t4)',
