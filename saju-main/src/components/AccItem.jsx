...
import { TIMING, shouldDropCap } from "../utils/constants.js";
...
export function FeedbackBtn({qIdx}){
  ...
  return(
    <div className="fb-wrap" role="group" aria-label="피드백" style={{ gap: 6, padding: '12px 0' }}>
      <span className="fb-label" style={{ fontSize: 'var(--xxs)' }}>이 이야기가 도움이 됐나요?</span>
    ...
    <div className="acc-item">
      ...
      <div id={bodyId} className={`acc-body${isOpen?' open':' closed'}`} role="region" aria-label={`Q${idx+1} 답변`}>
        <div className="acc-content">
          <p className={shouldDropCap(display) ? 'drop-cap' : ''}>{display}{isOpen&&!isDone&&<span className="typing-cursor" aria-hidden="true"/>}</p>
