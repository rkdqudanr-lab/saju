        ...
        <div className="compat-page">
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {otherProfiles.map((p, i) => (
                    <button key={`saved-${i}`}
                      ...
                      style={{ padding: '4px 10px', borderRadius: 20, border: '1px solid var(--acc)', background: 'var(--goldf)', color: 'var(--gold)', fontSize: 'var(--xxs)', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--ff)' }}>
                      ✦ {p.name || '이름없음'}
                    </button>
                  ))}
                </div>
                ...
                <div className="gender-group" style={{ marginTop: 6, marginBottom: 0 }}>
                  {['여성', '남성', '기타'].map(g => (
                    <button key={g} className={`gbtn ${partner.gender === g ? 'on' : ''}`}
                      ...
                      style={{ padding: '5px 4px', fontSize: 'var(--xxs)' }}>{g}</button>
                  ))}
                </div>
                {partnerSaju && (
          ...
          {partnerOk && saveOtherProfile && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <button
                ...
                style={{
                  padding: '7px 12px', borderRadius: 20, border: '1px solid var(--acc)',
                  background: 'var(--goldf)', color: 'var(--gold)', fontSize: 'var(--xxs)',
          ...
          {partnerOk && (
            <div className="compat-total" style={{ marginBottom: 'var(--sp2)' }}>
              ...
              <div className="kizmet-score" style={{ fontSize: '1.4rem', fontWeight: 800 }}>{compatScore}%</div>
              <div className="kizmet-bar" style={{ height: 6 }}><div className="kizmet-fill" style={{ width: `${compatScore}%`, boxShadow: '0 0 12px var(--gold)' }} /></div>
              {mySaju && partnerSaju && (
                ...
                <>
                  ...
                  {storyResult.story && (
                    <div style={{
                      ...
                      lineHeight: 1.9, marginBottom: 14, whiteSpace: 'pre-line',
                    }}>
                      {storyResult.story}
                    </div>
                  )}
