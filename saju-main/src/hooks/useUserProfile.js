...
const MOCK_USER = { id: 'test_user_777', nickname: '테스트 별님', profileImage: null };
const MOCK_FORM = { name: '테스트 별님', by: '1995', bm: '5', bd: '15', bh: '14.0000', gender: 'F', noTime: false };

export function useUserProfile() {
  const [user, setUser] = useState(() => (TEST_MODE ? MOCK_USER : getAuthUser()));
  const [profile, setProfile] = useState(DEFAULT_PROFILE);
  const [form, setForm] = useState(TEST_MODE ? MOCK_FORM : DEFAULT_FORM);
  ...
  useEffect(() => {
    ...
    if (TEST_MODE) {
      setConsentFlags({ marketing: true, privacy: true, terms: true });
      setOnboarded(true);
      ...
      try {
        ...
        if (supabase) {
          ...
          const { data: saved } = await (authClient || supabase)
            .from('users')
            .select('id, birth_year, birth_month, birth_day, birth_hour, gender, nickname, consent_flags, response_style, theme, onboarded, quiz_state, life_stage, font_size')
  ...
  useEffect(() => {
    ...
    (async () => {
      const [usersRes, profilesRes, othersRes] = await Promise.allSettled([
        client.from('users')
          .select('birth_year, birth_month, birth_day, birth_hour, gender, nickname, consent_flags, response_style, theme, onboarded, quiz_state, life_stage, font_size')
