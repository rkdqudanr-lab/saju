...
`src/components/landing/DailyMiniCard.jsx` — 홈의 오늘 별숨 카드. 점수는 `오늘의 별숨 지수` 배지로 표시하고, 조회 후 3×3 미니 대시보드가 보인다. 미니 버튼은 `onQuickAsk`로 채팅 프리필을 넘긴다.  
`ShareCardTemplate.jsx` — 인스타 피드용 4:5(1080×1350) 카드 저장 템플릿. 결과 공유는 `useAppHandlers.shareResult()`에서 `?from=share` 링크를 포함한다.
홈 오늘 카드 하단에는 진행률 바를 두지 않는다. 대시보드 카드와 `자세히 보기` CTA만으로 닫는다.
