import { getKstDateParts, getMonthJijiIndex, getSolarTermInfo } from '../../lib/jeolgi.js';
...
export function getTodayInfo() {
  ...
  const m = now.month;
  const d = now.day;
  const week = ["일", "월", "화", "수", "목", "금", "토"][now.weekIndex];
  const jeolgi = getSolarTermInfo(y, m, d, now.hour, now.minute).name;
  const lunarInfo = solarToLunar(y, m, d);
  const lunar = formatLunarDate(lunarInfo);
  const ilchin = _calcIlchin(y, m, d);
  return {
    solar: `${y}년 ${m}월 ${d}일 (${week}요일)`,
    lunar,
    lunarSupported: lunarInfo.supported,
    jeolgi,
    year: y,
    month: m,
    day: d,
