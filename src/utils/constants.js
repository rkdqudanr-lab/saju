...
export const DREAM_PROMPT = ({ dreamMood, dreamTags, moonPhaseLabel }) =>
  `[꿈 데이터]
꿈 속 요소: ${dreamTags && dreamTags.length > 0 ? dreamTags.join(', ') : '(없음)'}
깨어날 때의 감정: ${dreamMood || '(없음)'}
오늘의 달 위상: ${moonPhaseLabel || '(없음)'}

위 데이터와 아래 [꿈 내용]을 종합해서 응답 구조에 따라 해석해주세요.`;
...
후보 날짜와 간지 정보:
${candidateDates.map((d, i) => `${i + 1}. ${d.label} — 일진: ${d.sajuDesc} (사전 점수: ${d.score})`).join('\n')}

${sajuCtx ? `[사용자 성향 자료]\n${sajuCtx}` : ''}

위 데이터를 바탕으로 응답 구조에 따라 1순위·2순위를 결정하고 풀이해주세요.`;
...
${strokes ? `획수 합계: ${strokes}획` : '획수: 미제공'}
${sounds ? `이름 발음의 오행: ${sounds}` : ''}
${hanja ? `한자 제공됨 — 자의(字義)도 함께 풀이해주세요.` : '한자: 미제공'}

${sajuCtx ? `[사용자 성향 자료]\n${sajuCtx}` : ''}

위 데이터를 바탕으로 응답 구조에 따라 이름 풀이를 작성해주세요.
제공되지 않은 정보(획수·한자)는 절대로 임의로 만들지 마세요.`;
