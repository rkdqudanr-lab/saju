...
function PullBanner({ currentBP, pulling, onPull, cost1, cost10, title, subtitle, stats, accentColor, bgStyle, guarantee10Label }) {
  ...
  return (
    <div style={{ margin: '16px 0 0', borderRadius: 'var(--r2)', ...bgStyle, padding: '20px 16px', position: 'relative', overflow: 'hidden' }}>
      {[...Array(8)].map((_, i) => (
        <div key={i} style={{
          position: 'absolute',
  ...
  return (
    <div style={{ padding: '14px 0 0' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(90px, 1fr))', gap: 10 }}>
        {filtered.map(item => {
          const themeBg  = (item.category === 'theme' && item.colors?.bg2)      ? item.colors.bg2      : 'var(--bg2)';
          const themeAcc = (item.category === 'theme' && item.colors?.primary)   ? item.colors.primary  : cfg.border;
...
function InvCard({ item, isEquipped, onEquip, onUse }) {
  ...
  const themeBg  = (item.category === 'theme' && item.colors?.bg2)    ? item.colors.bg2    : 'var(--bg2)';
  const themeAcc = (item.category === 'theme' && item.colors?.primary) ? item.colors.primary : (isEquipped ? 'var(--acc)' : 'var(--line)');
  ...
  return (
    <div style={{
      ...
      position: 'relative',
      boxShadow: isEquipped ? `0 0 10px ${themeAcc === 'var(--line)' ? 'rgba(232,176,72,0.2)' : themeAcc}` : 'none',
      minHeight: 160,
    }}>
  ...
  return (
    <div className="page step-fade" style={{ paddingBottom: 'calc(80px + env(safe-area-inset-bottom, 16px))', alignItems: 'stretch' }}>
      ...
      {activeTab === 'spirit' && (
        <>
          {/* 우주/사주 서브탭 */}
          <div style={{ display: 'flex', margin: '14px 0 0', background: 'var(--bg2)', borderRadius: 'var(--r1)', padding: 4, border: '1px solid var(--line)' }}>
      ...
      {(activeTab === 'theme' || activeTab === 'avatar' || activeTab === 'effect') && (() => {
        ...
        return (
          <>
            {/* 장착 현황 */}
            {equippedItem ? (
              <div style={{ margin: '14px 0 0', padding: '10px 12px', background: 'rgba(232,176,72,0.06)', border: '1px solid var(--acc)', borderRadius: 'var(--r1)', display: 'flex', alignItems: 'center', gap: 10 }}>
