...
function renderBoldText(text) {
  ...
  // 2. 핵심 구문 ** 마킹
  // 공백 없는 어간(동사/형용사)만 잡아 짧고 정확하게 굵게 처리

  // ① 가능성/주의: [동사어간 2-8자]질/을 수 있어요 — "빨라질 수 있어요"
  t = t.replace(/([가-힣]{2,8}[질을]\s*수\s+있어요)/g, m => `**${m.trim()}**`);

  // ② 권고: [동사어간 1-6자]면 좋아요 — "피하면 좋아요", "쉬면 좋아요"
  t = t.replace(/([가-힣]{1,6}면\s*좋아요)/g, m => `**${m.trim()}**`);

  // ③ 추천: [관형형 2-6자] 편이 좋아요/유리해요 — "읽는 편이 유리해요"
  t = t.replace(/([가-힣]{2,6}\s편이\s*(?:좋아요|유리해요))/g, m => `**${m.trim()}**`);

  // ④ 조건-결과: [동사어간 2-6자]수록 [결과 2-8자] — "정리할수록 안정감이 커져요"
  t = t.replace(/([가-힣]{2,6}수록\s*[가-힣\s]{2,8}(?:커져요|열려요|올라요|됩니다|좋아요))/g, m => `**${m.trim()}**`);

  // ⑤ 축 특성: X쪽은 Y하고/하며 — "창의 쪽은 빠르게 반응하고"
