...
function renderBoldText(text) {
  ...
  // ① ~ㄹ/~을 수 있어요 (가능성 서술) — "말과 행동이 빨라질 수 있어요"
  t = t.replace(/([^.。,，]*?[가-힣]{1,}[질을]?\s*수\s+있어요)/g, m => `**${m.trim()}**`);

  // ② 축 특성: "X 쪽은 비교적/빠르게 Y하고/Y하며" — "창의 쪽은 비교적 빠르게 반응하고"
  t = t.replace(/([가-힣]{1,6}\s*쪽은\s*[가-힣\s]{3,25}(?:하고|하며|반응하고|올라오고|작동하고|나타나고))/g, m => `**${m.trim()}**`);

  // ③ 행동 명사구: "~기" + 방향 — "할 일을 세 가지로 줄이기"
  t = t.replace(/([가-힣\s]{4,25}기)(?=\s*(?:쪽으로|으로|를|이\s|가\s))/g, m => `**${m.trim()}**`);

  // ④ 편이 좋아요/유리해요: 추천 행동 — "섬세하게 읽는 편이 유리해요"
  t = t.replace(/([^.。]{4,35}(?:편이\s*좋아요|편이\s*유리해요))/g, m => `**${m.trim()}**`);

  // ⑤ ~면 좋아요: 권고 — "피하면 좋아요"
  t = t.replace(/([^.。]{4,30}면\s*좋아요)/g, m => `**${m.trim()}**`);

  // ⑥ ~수록 결과(커져요/열려요/됩니다): 조건-결과 — "흐름을 정리할수록 안정감이 커져요"
  t = t.replace(/([^.。]{4,35}수록\s*[가-힣\s]{2,15}(?:커져요|열려요|올라요|됩니다|좋아요))/g, m => `**${m.trim()}**`);
