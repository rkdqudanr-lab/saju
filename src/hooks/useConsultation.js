...
export function useConsultation(buildCtx, formOk) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        const res  = await fetch('/api/ask', {
          ...
          body: JSON.stringify({ userMessage, context: buildCtx(), isChat: opts.isChat || false, isReport: opts.isReport || false, isDailyCard: opts.isDailyCard || false }),
        });
  ...
  const askDailyHoroscope = useCallback(async () => {
    ...
    try {
      const ans = await callApi('오늘 하루 나의 별숨은?', { isDailyCard: true });
